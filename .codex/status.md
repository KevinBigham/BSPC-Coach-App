...
- Validation so far: `npm run typecheck`, `npm run lint:errors`, `npm test -- --runInBand` (950 / 99), `npm --prefix functions test -- --runInBand` (106 / 18), `npm --prefix functions run build`, `npm run quality:dead-code`, and focused seed/picker/audio/video/function prompt tests all passed. Seed safety check refused when `EXPO_PUBLIC_BSPC_ENV!=demo`, as intended.

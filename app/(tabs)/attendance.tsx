...
export default function AttendanceScreen() {
  ...
  const getRecord = useCallback(
    (swimmerId: string) => todayRecords.find((r) => r.swimmerId === swimmerId && !r.departedAt)
  ...
  const handleCheckout = async () => {
    ...
    await checkOut(
      checkoutModal.recordId,
      checkoutStatus,
      checkoutNote.trim() || undefined
    );
    ...
    return (
      ...
      >
        ...
        <View style={styles.swimmerInfo}>
          <Text style={styles.swimmerName}>{item.firstName} {item.lastName}</Text>
          <View style={styles.metaRow}>
            ...
            {record && (
              <Text style={styles.timeText}>
                {isPresent && record.arrivedAt ? `In ${formatClockTime(record.arrivedAt instanceof Date ? record.arrivedAt : new Date())}` : ''}
                {hasDeparted ? `Departed${record.status && record.status !== 'normal' ? ` (${record.status})` : ''}` : ''}
              </Text>
            )}
          </View>
        </View>
              ...
              <View style={styles.statusGrid}>
                {STATUSES.map((s) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.statusChip,
                      checkoutStatus === s.value && { backgroundColor: s.color, borderColor: s.color },
                    ]}
...
const styles = StyleSheet.create({
  ...
  // Scorebug Header
  headerBug: { backgroundColor: colors.bgDeep, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingVertical: spacing.lg, paddingHorizontal: spacing.xl, borderBottomWidth: 2, borderBottomColor: colors.purple },
  headerLeft: { alignItems: 'flex-start' },
  headerRight: { alignItems: 'flex-end', gap: spacing.xs },
  exportBtn: { paddingHorizontal: spacing.sm, paddingVertical: 2, borderRadius: borderRadius.sm, borderWidth: 1, borderColor: colors.accent },
  exportBtnText: { fontFamily: fontFamily.bodySemi, fontSize: fontSize.xs, color: colors.accent, letterSpacing: 1 },
  ...
  headerLabel: { fontFamily: fontFamily.pixel, fontSize: fontSize.pixel, color: colors.gold, letterSpacing: 1, marginTop: spacing.xs },
  presentCount: { fontFamily: fontFamily.stat, fontSize: 48, color: colors.success, lineHeight: 52 },
  presentSep: { fontFamily: fontFamily.heading, fontSize: fontSize.xxl, color: colors.textSecondary, marginHorizontal: spacing.xs },
  totalCount: { fontFamily: fontFamily.stat, fontSize: 48, color: colors.textSecondary, lineHeight: 52 },
  // Filters
  filterContainer: { backgroundColor: colors.bgElevated, borderBottomWidth: 1, borderBottomColor: colors.border },
  ...
  batchBtn: { marginHorizontal: spacing.lg, marginBottom: spacing.sm, padding: spacing.sm, borderRadius: borderRadius.sm, backgroundColor: colors.purple, alignItems: 'center' },
  batchBtnText: { fontFamily: fontFamily.bodySemi, fontSize: fontSize.sm, color: colors.text, letterSpacing: 1 },
  // List
  list: { padding: spacing.lg, paddingBottom: spacing.xxxl },
  swimmerRow: { flexDirection: 'row', alignItems: 'center', backgroundColor: colors.bgDeep, padding: spacing.lg, borderRadius: borderRadius.md, marginBottom: spacing.sm, borderWidth: 1, borderColor: colors.border },
  ...
  tapHint: { fontFamily: fontFamily.pixel, fontSize: fontSize.pixel, color: colors.accent, letterSpacing: 1 },
  // Empty
  ...
  emptySubtitle: { fontFamily: fontFamily.body, fontSize: fontSize.sm, color: colors.textSecondary, marginTop: spacing.sm },
  // Checkout Modal
  ...
  modalTitle: { fontFamily: fontFamily.heading, fontSize: fontSize.xxl, color: colors.text, letterSpacing: 1 },
  modalSubtitle: { fontFamily: fontFamily.bodySemi, fontSize: fontSize.md, color: colors.accent, marginTop: spacing.xs, marginBottom: spacing.lg },
  modalLabel: { fontFamily: fontFamily.bodySemi, fontSize: fontSize.xs, color: colors.textSecondary, letterSpacing: 1, marginTop: spacing.lg, marginBottom: spacing.sm },
  statusGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: spacing.sm },
  statusChip: { paddingHorizontal: spacing.md, paddingVertical: spacing.sm, borderRadius: borderRadius.sm, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.bgBase },
  statusChipText: { fontFamily: fontFamily.bodySemi, fontSize: fontSize.sm, color: colors.text },
  modalInput: { borderWidth: 1, borderColor: colors.border, borderRadius: borderRadius.sm, padding: spacing.md, fontSize: fontSize.md, fontFamily: fontFamily.body, color: colors.text, backgroundColor: colors.bgBase, minHeight: 60, textAlignVertical: 'top' },
  ...
  modalCheckoutBtn: { flex: 1, padding: spacing.md, borderRadius: borderRadius.sm, backgroundColor: colors.purple, alignItems: 'center' },

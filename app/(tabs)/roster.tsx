...
export default function RosterScreen() {
  ...
  const handleActivate = (swimmer: Swimmer & { id: string }) => {
    Alert.alert('Activate Swimmer', `Move ${swimmer.firstName} ${swimmer.lastName} back to active?`, [
  ...
  const renderSwimmer = ({ item }: { item: Swimmer & { id: string } }) => (
    ...
    >
      ...
      <View style={styles.swimmerInfo}>
        <Text style={styles.swimmerName}>{item.lastName}, {item.firstName}</Text>
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            <Text style={styles.emptyTitle}>
              {swimmers.length === 0 ? (showInactive ? 'NO INACTIVE SWIMMERS' : 'NO SWIMMERS YET') : 'NO RESULTS'}
            </Text>
            <Text style={styles.emptySubtitle}>
              {swimmers.length === 0
                ? (showInactive ? 'All swimmers are currently active' : 'Tap + to add your first swimmer')
...
const styles = StyleSheet.create({
  ...
  // Count Header
  countHeader: { backgroundColor: colors.bgDeep, paddingVertical: spacing.md, paddingHorizontal: spacing.xl, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderBottomWidth: 1, borderBottomColor: colors.border },
  exportBtn: { paddingHorizontal: spacing.md, paddingVertical: spacing.xs, borderRadius: borderRadius.sm, borderWidth: 1, borderColor: colors.accent },
  ...
  countLabel: { fontFamily: fontFamily.pixel, fontSize: fontSize.pixel, color: colors.gold, letterSpacing: 1 },
  // Toggle
  toggleRow: { flexDirection: 'row', paddingHorizontal: spacing.lg, paddingTop: spacing.sm, gap: spacing.sm, backgroundColor: colors.bgElevated },
  toggleBtn: { flex: 1, paddingVertical: spacing.sm, alignItems: 'center', borderRadius: borderRadius.sm, backgroundColor: colors.bgDeep, borderWidth: 1, borderColor: colors.border },
  toggleBtnActive: { backgroundColor: colors.purple, borderColor: colors.purpleLight },
  toggleBtnText: { fontFamily: fontFamily.bodySemi, fontSize: fontSize.sm, color: colors.textSecondary },
  toggleBtnTextActive: { color: colors.text },
  // Search
  searchRow: { flexDirection: 'row', padding: spacing.lg, paddingBottom: spacing.sm, backgroundColor: colors.bgElevated, gap: spacing.sm },
  searchInput: { flex: 1, backgroundColor: colors.bgDeep, borderRadius: borderRadius.sm, padding: spacing.md, fontSize: fontSize.md, fontFamily: fontFamily.body, color: colors.text, borderWidth: 1, borderColor: colors.border },
  sortBtn: { backgroundColor: colors.bgDeep, borderRadius: borderRadius.sm, paddingHorizontal: spacing.md, justifyContent: 'center', borderWidth: 1, borderColor: colors.border },
  sortBtnText: { fontFamily: fontFamily.bodySemi, fontSize: fontSize.xs, color: colors.accent, letterSpacing: 1 },
  // Filters
  ...
  avatar: { width: 44, height: 44, borderRadius: 22, backgroundColor: colors.purple, alignItems: 'center', justifyContent: 'center', marginRight: spacing.md },
  ...
  swimmerMeta: { flexDirection: 'row', alignItems: 'center', gap: spacing.sm, marginTop: spacing.xs },
  ...
  // Hint
  hintBar: { padding: spacing.sm, backgroundColor: colors.bgElevated, alignItems: 'center', borderTopWidth: 1, borderTopColor: colors.border },
  hintText: { fontFamily: fontFamily.body, fontSize: fontSize.xs, color: colors.textSecondary, fontStyle: 'italic' },
  // FAB
  fab: { position: 'absolute', bottom: spacing.xl, right: spacing.xl, width: 56, height: 56, borderRadius: 28, backgroundColor: colors.accent, alignItems: 'center', justifyContent: 'center', elevation: 4, shadowColor: colors.accent, shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.4, shadowRadius: 4 },

...
export default function RelayBuilderScreen() {
  ...
  const relayEvents = meet?.events.filter((e) => e.isRelay).map((e) => e.name) || RELAY_EVENTS.map(String);
    ...
    <View style={styles.container}>
      ...
      <View style={styles.topBar}>
        ...
        <TouchableOpacity onPress={handleSave} disabled={saving || selectedSwimmers.length !== 4}>
          <Text style={[styles.saveText, (saving || selectedSwimmers.length !== 4) && { opacity: 0.5 }]}>
            {saving ? 'SAVING...' : 'SAVE'}
          </Text>
        </TouchableOpacity>
      </View>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.eventRow}>
          {relayEvents.map((e) => (
            ...
            >
              <Text style={[styles.eventChipText, selectedEvent === e && styles.eventChipTextActive]}>{e}</Text>
            </TouchableOpacity>
          ))}
        </View>
        ...
        <View style={styles.teamRow}>
          {['BSPC A', 'BSPC B', 'BSPC C'].map((t) => (
            ...
            >
              <Text style={[styles.teamChipText, teamName === t && styles.teamChipTextActive]}>{t}</Text>
            </TouchableOpacity>
          ))}
        </View>
        ...
        <View style={styles.previewCard}>
          <Text style={styles.previewTitle}>{selectedEvent} — {teamName}</Text>
          {buildLegs().map((leg) => (
          ...
          {selectedSwimmers.length < 4 && (
            <Text style={styles.previewHint}>
              Select {4 - selectedSwimmers.length} more swimmer{4 - selectedSwimmers.length !== 1 ? 's' : ''}
            </Text>
          )}
        </View>
            ...
            return (
              ...
              >
                ...
                <View style={styles.swimmerInfo}>
                  <Text style={styles.swimmerName}>{swimmer.lastName}, {swimmer.firstName}</Text>
...
const styles = StyleSheet.create({
  ...
  // Top Bar
  topBar: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', padding: spacing.lg, backgroundColor: colors.bgDeep, borderBottomWidth: 2, borderBottomColor: colors.gold },
  ...
  label: { fontFamily: fontFamily.bodySemi, fontSize: fontSize.xs, color: colors.textSecondary, letterSpacing: 1, marginTop: spacing.lg, marginBottom: spacing.sm },
  // Event
  ...
  teamChip: { flex: 1, padding: spacing.md, borderRadius: borderRadius.sm, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.bgDeep, alignItems: 'center' },
  teamChipActive: { backgroundColor: colors.purple, borderColor: colors.purpleLight },
  teamChipText: { fontFamily: fontFamily.bodySemi, fontSize: fontSize.sm, color: colors.textSecondary },
  teamChipTextActive: { color: colors.text },
  // Preview
  previewCard: { backgroundColor: colors.bgDeep, borderRadius: borderRadius.md, padding: spacing.lg, borderWidth: 2, borderColor: colors.gold, marginTop: spacing.lg },
  previewTitle: { fontFamily: fontFamily.heading, fontSize: fontSize.xl, color: colors.gold, letterSpacing: 1, marginBottom: spacing.sm },
  previewLeg: { fontFamily: fontFamily.body, fontSize: fontSize.sm, color: colors.text, paddingVertical: 2 },
  previewHint: { fontFamily: fontFamily.bodySemi, fontSize: fontSize.xs, color: colors.textSecondary, marginTop: spacing.sm },
  // Swimmer List
  swimmerRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: spacing.sm, paddingHorizontal: spacing.md, borderBottomWidth: 1, borderBottomColor: colors.borderLight },
  swimmerRowActive: { backgroundColor: 'rgba(74, 14, 120, 0.2)' },
  selBadge: { width: 28, height: 28, borderRadius: 14, borderWidth: 2, borderColor: colors.border, alignItems: 'center', justifyContent: 'center', marginRight: spacing.md },

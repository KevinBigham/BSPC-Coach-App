...
import {
  parseSDIF,
  matchSwimmers,
  type SDIFParseResult,
  type MatchResult,
} from '../src/services/sdifImport';
...
export default function MeetImportScreen() {
  ...
  const [importResult, setImportResult] = useState<{ imported: number; prs: number; skipped: number; errors: string[] } | null>(null);
  ...
  const handleParse = () => {
    ...
    if (parsed.records.length === 0) {
      Alert.alert('No Results', `No swim times found in the data.${parsed.errors.length > 0 ? ` Errors: ${parsed.errors.length}` : ''}`);
      return;
    }
            ...
            <>
              <View style={styles.card}>
                ...
                <Text style={styles.cardDesc}>
                  Import swim times from SDIF (.sd3) or Hy-Tek (.hy3) files. Pick a file or paste the data below.
                </Text>
                ...
                <View style={styles.formatRow}>
                  {(['auto', 'sdif', 'hy3'] as FileFormat[]).map((f) => (
                    ...
                    >
                      <Text style={[styles.formatChipText, fileFormat === f && styles.formatChipTextActive]}>
                  ...
                  <>
                    ...
                    <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                      <View style={styles.formatRow}>
                        ...
                        >
                          <Text style={[styles.formatChipText, !selectedMeetId && styles.formatChipTextActive]}>NONE</Text>
                        </TouchableOpacity>
                        {meets.slice(0, 10).map((m) => (
                          <TouchableOpacity
                            key={m.id}
                            style={[styles.formatChip, selectedMeetId === m.id && styles.formatChipActive]}
                            onPress={() => setSelectedMeetId(m.id)}
                          >
                            <Text
                              style={[styles.formatChipText, selectedMeetId === m.id && styles.formatChipTextActive]}
                              numberOfLines={1}
                            >
                              {m.name}
                            </Text>
            ...
            <>
              ...
              <View style={styles.card}>
                ...
                <View style={styles.meetMeta}>
                  {parseResult.meetDate && <Text style={styles.meetMetaText}>{parseResult.meetDate}</Text>}
              ...
              {parseResult.errors.length > 0 && (
                <View style={[styles.card, { borderColor: colors.warning }]}>
                  <Text style={[styles.cardTitle, { color: colors.warning }]}>WARNINGS ({parseResult.errors.length})</Text>
                  ...
                  return (
                    <View key={i} style={styles.resultRow}>
                      ...
                      <View style={[styles.matchBadge, { borderColor: badge.color }]}>
                        <Text style={[styles.matchBadgeText, { color: badge.color }]}>{badge.label}</Text>
                      </View>
                    </View>
                  );
                })}
              ...
              <View style={styles.centeredCard}>
                ...
                <View style={styles.doneStats}>
                  <View style={styles.doneStat}>
                    <Text style={[styles.doneStatNum, { color: colors.accent }]}>{importResult.imported}</Text>
                    <Text style={styles.doneStatLabel}>IMPORTED</Text>
                  </View>
                  <View style={styles.doneStat}>
                    <Text style={[styles.doneStatNum, { color: colors.gold }]}>{importResult.prs}</Text>
                    <Text style={styles.doneStatLabel}>NEW PRs</Text>
                  </View>
                  <View style={styles.doneStat}>
                    <Text style={[styles.doneStatNum, { color: colors.textSecondary }]}>{importResult.skipped}</Text>
...
const styles = StyleSheet.create({
  ...
  cardTitle: { fontFamily: fontFamily.heading, fontSize: fontSize.xl, color: colors.text, letterSpacing: 1, marginBottom: spacing.sm },
  cardDesc: { fontFamily: fontFamily.body, fontSize: fontSize.sm, color: colors.textSecondary, lineHeight: 20, marginBottom: spacing.lg },

  // Format / Meet Linking
  fieldLabel: { fontFamily: fontFamily.bodySemi, fontSize: fontSize.xs, color: colors.textSecondary, letterSpacing: 1, marginBottom: spacing.sm, marginTop: spacing.md },
  formatRow: { flexDirection: 'row', gap: spacing.xs, marginBottom: spacing.md },
  formatChip: { paddingHorizontal: spacing.md, paddingVertical: spacing.sm, borderRadius: borderRadius.sm, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.bgBase },
  formatChipActive: { backgroundColor: colors.purple, borderColor: colors.purpleLight },
  formatChipText: { fontFamily: fontFamily.bodySemi, fontSize: fontSize.xs, color: colors.textSecondary },
  formatChipTextActive: { color: colors.text },

  // Input
  pickFileBtn: { backgroundColor: colors.purple, padding: spacing.md, borderRadius: borderRadius.sm, alignItems: 'center', marginBottom: spacing.md },
  pickFileBtnText: { fontFamily: fontFamily.bodySemi, fontSize: fontSize.md, color: colors.text, letterSpacing: 1 },
  orText: { fontFamily: fontFamily.body, fontSize: fontSize.sm, color: colors.textSecondary, textAlign: 'center', marginBottom: spacing.md },
  textArea: { borderWidth: 1, borderColor: colors.border, borderRadius: borderRadius.sm, padding: spacing.md, fontFamily: fontFamily.statMono, fontSize: fontSize.xs, color: colors.text, backgroundColor: colors.bgBase, minHeight: 150, textAlignVertical: 'top' },

  // Meet Meta
  meetMeta: { flexDirection: 'row', gap: spacing.md },
  meetMetaText: { fontFamily: fontFamily.statMono, fontSize: fontSize.sm, color: colors.textSecondary },

  // Summary
  summaryRow: { flexDirection: 'row', gap: spacing.md },
  summaryCard: { flex: 1, backgroundColor: colors.bgDeep, borderRadius: borderRadius.lg, padding: spacing.lg, alignItems: 'center', borderWidth: 1 },
  summaryNum: { fontFamily: fontFamily.stat, fontSize: fontSize.xxxl },
  summaryLabel: { fontFamily: fontFamily.bodySemi, fontSize: fontSize.xs, color: colors.textSecondary, letterSpacing: 1 },

  // Results
  resultRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: spacing.sm, borderBottomWidth: 1, borderBottomColor: colors.borderLight },
  resultLeft: { flex: 1 },
  resultName: { fontFamily: fontFamily.bodySemi, fontSize: fontSize.sm, color: colors.text },
  resultEvent: { fontFamily: fontFamily.statMono, fontSize: fontSize.xs, color: colors.textSecondary, marginTop: 2 },
  matchBadge: { paddingHorizontal: spacing.sm, paddingVertical: 2, borderRadius: borderRadius.xs, borderWidth: 1, backgroundColor: 'rgba(0,0,0,0.3)' },
  matchBadgeText: { fontFamily: fontFamily.pixel, fontSize: fontSize.pixel },
  moreText: { fontFamily: fontFamily.body, fontSize: fontSize.sm, color: colors.textSecondary, textAlign: 'center', paddingVertical: spacing.md },

  // Errors
  errorLine: { fontFamily: fontFamily.statMono, fontSize: fontSize.xs, color: colors.warning, marginTop: 2 },

  // Actions
  actionRow: { flexDirection: 'row', gap: spacing.md },
  primaryBtn: { flex: 1, backgroundColor: colors.purple, padding: spacing.lg, borderRadius: borderRadius.sm, alignItems: 'center' },
  primaryBtnText: { fontFamily: fontFamily.bodySemi, fontSize: fontSize.md, color: colors.text, letterSpacing: 1 },
  secondaryBtn: { padding: spacing.lg, borderRadius: borderRadius.sm, borderWidth: 1, borderColor: colors.border, alignItems: 'center' },
  secondaryBtnText: { fontFamily: fontFamily.bodySemi, fontSize: fontSize.md, color: colors.textSecondary },

  // Importing/Done
  centeredCard: { backgroundColor: colors.bgDeep, borderRadius: borderRadius.lg, padding: spacing.xxl, alignItems: 'center', borderWidth: 1, borderColor: colors.border },
  importingText: { fontFamily: fontFamily.heading, fontSize: fontSize.xl, color: colors.accent, letterSpacing: 1, marginTop: spacing.lg },
  doneTitle: { fontFamily: fontFamily.heading, fontSize: fontSize.xxl, color: colors.success, letterSpacing: 1, marginBottom: spacing.lg },
  ...
  doneStatLabel: { fontFamily: fontFamily.bodySemi, fontSize: fontSize.xs, color: colors.textSecondary, letterSpacing: 1 },

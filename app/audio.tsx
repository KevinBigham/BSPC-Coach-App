...
export default function AudioScreen() {
  ...
  const handleUpload = async () => {
    if (!recordingRef.current || !coach) return;
      ...
      <View style={styles.container}>
        ...
        {!isOnline && (
          <View style={styles.offlineBanner}>
            <Text style={styles.offlineBannerText}>OFFLINE — recordings will upload when reconnected</Text>
          </View>
        )}
            ...
            sessions.map((session) => {
              ...
              return (
                <View key={session.id} style={styles.sessionCard}>
                  ...
                  <Text style={styles.sessionCoach}>{session.coachName} • {formatRelativeTime(createdAt)}</Text>
                </View>
              );
            })
          )}
...
const styles = StyleSheet.create({
  ...
  offlineBanner: { backgroundColor: colors.error, paddingVertical: spacing.sm, paddingHorizontal: spacing.lg, alignItems: 'center' },
  offlineBannerText: { fontFamily: fontFamily.bodySemi, fontSize: fontSize.xs, color: colors.text, letterSpacing: 1 },
  scrollView: { flex: 1 },
  scrollContent: { padding: spacing.lg, paddingBottom: spacing.xxxl, gap: spacing.md },

  // Recorder Card
  recorderCard: { backgroundColor: colors.bgDeep, borderRadius: borderRadius.lg, padding: spacing.xl, borderWidth: 1, borderColor: colors.border },
  cardTitle: { fontFamily: fontFamily.heading, fontSize: fontSize.xxl, color: colors.text, letterSpacing: 1, marginBottom: spacing.md },
  durationRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: spacing.sm, marginBottom: spacing.lg },
  ...
  groupPickerLabel: { fontFamily: fontFamily.bodySemi, fontSize: fontSize.xs, color: colors.textSecondary, letterSpacing: 1, marginBottom: spacing.sm },
  groupRow: { flexDirection: 'row', gap: spacing.xs },
  groupChip: { paddingHorizontal: spacing.md, paddingVertical: spacing.sm, borderRadius: borderRadius.sm, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.bgBase },
  groupChipActive: { backgroundColor: colors.purple, borderColor: colors.purpleLight },
  groupChipText: { fontFamily: fontFamily.bodySemi, fontSize: fontSize.xs, color: colors.textSecondary },
  groupChipTextActive: { color: colors.text },

  // Controls
  controls: { alignItems: 'center', gap: spacing.md },
  recordBtn: { flexDirection: 'row', alignItems: 'center', backgroundColor: colors.error, paddingHorizontal: spacing.xl, paddingVertical: spacing.md, borderRadius: borderRadius.full, gap: spacing.sm },
  recordBtnDot: { width: 12, height: 12, borderRadius: 6, backgroundColor: colors.text },
  recordBtnText: { fontFamily: fontFamily.bodySemi, fontSize: fontSize.md, color: colors.text, letterSpacing: 1 },
  stopBtn: { flexDirection: 'row', alignItems: 'center', backgroundColor: colors.bgBase, paddingHorizontal: spacing.xl, paddingVertical: spacing.md, borderRadius: borderRadius.full, gap: spacing.sm, borderWidth: 2, borderColor: colors.text },
  stopBtnSquare: { width: 12, height: 12, backgroundColor: colors.error, borderRadius: 2 },
  stopBtnText: { fontFamily: fontFamily.bodySemi, fontSize: fontSize.md, color: colors.text, letterSpacing: 1 },
  uploadBtn: { backgroundColor: colors.purple, paddingHorizontal: spacing.xl, paddingVertical: spacing.md, borderRadius: borderRadius.sm, width: '100%', alignItems: 'center' },
  uploadBtnText: { fontFamily: fontFamily.bodySemi, fontSize: fontSize.md, color: colors.text, letterSpacing: 1 },
  discardBtn: { paddingVertical: spacing.sm },
  discardBtnText: { fontFamily: fontFamily.body, fontSize: fontSize.sm, color: colors.textSecondary, textDecorationLine: 'underline' },
  ...
  progressText: { fontFamily: fontFamily.statMono, fontSize: fontSize.sm, color: colors.textSecondary },

  // Sessions
  sessionsHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  sessionsTitle: { fontFamily: fontFamily.heading, fontSize: fontSize.xl, color: colors.text, letterSpacing: 1 },
  ...
  sessionCard: { backgroundColor: colors.bgDeep, borderRadius: borderRadius.md, padding: spacing.lg, borderWidth: 1, borderColor: colors.border },
  ...
  sessionMeta: { fontFamily: fontFamily.statMono, fontSize: fontSize.xs, color: colors.textSecondary, marginTop: 2 },

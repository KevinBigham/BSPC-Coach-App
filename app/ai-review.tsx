...
export default function AIReviewScreen() {
  ...
  const handleApproveAll = async () => {
    ...
    Alert.alert(
      'Approve All',
      `Post all ${drafts.length} AI observations to swimmer profiles?`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Approve All',
        ...
        <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent}>
          {drafts.length === 0 ? (
            <View style={styles.emptyContainer}>
              ...
              <Text style={styles.emptyHint}>No AI drafts pending review. Record audio during practice to generate AI observations.</Text>
            </View>
          ) : (
            drafts.map((draft) => {
                    ...
                    <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.tagScroll}>
                      <View style={styles.tagRow}>
                        {NOTE_TAGS.map((tag) => (
                          ...
                          >
                            <Text style={[styles.tagText, editTags.includes(tag) && styles.tagTextActive]}>{tag}</Text>
                          </TouchableOpacity>
                        ))}
                      </View>
                    </ScrollView>
...
const styles = StyleSheet.create({
  ...
  // Header
  header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: spacing.lg, backgroundColor: colors.bgDeep, borderBottomWidth: 1, borderBottomColor: colors.border },
  headerCount: { fontFamily: fontFamily.stat, fontSize: fontSize.xxxl, color: colors.gold },
  headerLabel: { fontFamily: fontFamily.bodySemi, fontSize: fontSize.xs, color: colors.textSecondary, letterSpacing: 1 },
  approveAllBtn: { backgroundColor: colors.purple, paddingHorizontal: spacing.lg, paddingVertical: spacing.md, borderRadius: borderRadius.sm },
  approveAllBtnText: { fontFamily: fontFamily.bodySemi, fontSize: fontSize.sm, color: colors.text, letterSpacing: 1 },

  // Empty
  emptyContainer: { alignItems: 'center', paddingVertical: spacing.xxxl },
  emptyTitle: { fontFamily: fontFamily.pixel, fontSize: fontSize.pixelLg, color: colors.success, letterSpacing: 2, marginBottom: spacing.md },
  emptyHint: { fontFamily: fontFamily.body, fontSize: fontSize.sm, color: colors.textSecondary, textAlign: 'center', lineHeight: 20 },

  // Draft Card
  draftCard: { backgroundColor: colors.bgDeep, borderRadius: borderRadius.lg, padding: spacing.lg, borderWidth: 1, borderColor: colors.border },
  draftSwimmer: { fontFamily: fontFamily.heading, fontSize: fontSize.xl, color: colors.text, letterSpacing: 1, marginBottom: spacing.xs },
  ...
  confidenceText: { fontFamily: fontFamily.statMono, fontSize: fontSize.xs, color: colors.textSecondary },
  draftObservation: { fontFamily: fontFamily.body, fontSize: fontSize.md, color: colors.text, lineHeight: 22 },
  tapToEdit: { fontFamily: fontFamily.body, fontSize: fontSize.xs, color: colors.textSecondary, fontStyle: 'italic', marginTop: spacing.xs },
  editInput: { borderWidth: 1, borderColor: colors.accent, borderRadius: borderRadius.sm, padding: spacing.md, fontFamily: fontFamily.body, fontSize: fontSize.md, color: colors.text, backgroundColor: colors.bgBase, minHeight: 80, textAlignVertical: 'top' },
  ...
  rejectBtn: { flex: 1, padding: spacing.md, borderRadius: borderRadius.sm, borderWidth: 1, borderColor: colors.error, alignItems: 'center' },
  rejectBtnText: { fontFamily: fontFamily.bodySemi, fontSize: fontSize.sm, color: colors.error, letterSpacing: 1 },
  approveBtn: { flex: 1, padding: spacing.md, borderRadius: borderRadius.sm, backgroundColor: colors.purple, alignItems: 'center' },
  approveBtnText: { fontFamily: fontFamily.bodySemi, fontSize: fontSize.sm, color: colors.text, letterSpacing: 1 },
  draftGroup: { fontFamily: fontFamily.body, fontSize: fontSize.xs, color: colors.textSecondary, marginTop: spacing.sm, fontStyle: 'italic' },
});

...
describe('demoReadiness helpers', () => {
  ...
  it('shows missing attendance context on roster facts', () => {
    expect(
      buildRosterDemoFacts(makeSwimmer({ mediaConsent: { granted: true, date: new Date() } })),
    ).toEqual([{ label: '30D --', tone: 'neutral' }]);
  });
  ...
  it('chooses the safest next practice action before lower-priority cues', () => {
    const snapshot = buildProfileCoachSnapshot({
      swimmer: makeSwimmer({ doNotPhotograph: true }),
      notes: [],
      times: [],
      attendance: [],
      goals: [makeGoal()],
